...
<div class="table-holder body-holder filtered-clients">
    <table class="table table-bordered table-hover">
        ...
        <?php foreach($clients_rows as $row): ?>
            <tr>
                <td><a href="/ajax/clientmodalsell/<?php echo $row['id']; ?>" data-toggle="modal" data-target=".cust-info" class="load-modal-client"><?php echo $row['type'] == 0 ? $row['name'].' '.$row['surname'] : $row['company_name']; ?></a></td>
...
<div class="new-cust-btn-holder">
    <button data-toggle="modal" data-target=".new-customer-physical"><?php echo $this->labels['new customer']; ?><span class="glyphicon glyphicon-plus-sign"></span></button>
</div>
